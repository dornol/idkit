...
- **UUID v7 `rand_a` bit semantics**: the 12 `rand_a` bits now carry a monotonic counter (RFC 9562 §6.2 Method 1), not random data. Generated UUIDs are still spec-compliant v7 UUIDs, but callers that depended on `rand_a` being fully random must re-evaluate their assumptions.
