...
open class UlidIdGenerator : IdGenerator<String> {
    ...
    final override fun nextId(): String {
        ...
        if (now > lastTimestamp) {
            ...
            randomHi = random.nextLong() and RANDOM_HI_MASK
            randomLo = random.nextLong()
        } else {
    ...
    private companion object {
        ...
        private fun encode(timestamp: Long, randomHi: Long, randomLo: Long): String {
            ...
            var ts = timestamp
            for (i in 9 downTo 0) {
                chars[i] = ALPHABET[(ts and MASK_5_BITS).toInt()]
                ts = ts ushr 5
            }

            // chars 25..14: 12 chars from randomLo's low 60 bits.
            var rl = randomLo
            for (i in 25 downTo 14) {
                chars[i] = ALPHABET[(rl and MASK_5_BITS).toInt()]
                rl = rl ushr 5
            }

            // char 13: 4 high bits of randomLo + 1 low bit of randomHi.
            var rh = randomHi
            val char13 = ((rh and 0x1L) shl 4) or rl
            chars[13] = ALPHABET[char13.toInt()]
            rh = rh ushr 1

            // chars 12..10: 3 chars from randomHi's remaining 15 bits.
            for (i in 12 downTo 10) {
                chars[i] = ALPHABET[(rh and MASK_5_BITS).toInt()]
                rh = rh ushr 5
            }
